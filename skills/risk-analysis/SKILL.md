...
6. **優先度付けと出所レイヤの明示**: `likelihood × impact` で優先順位を決め、
   ...
   パターンからの仮説である旨を、エンベロープの `assumptions[]` に
   `{field, value, reason}` 形式で記録する。リスク項目自体には `assumption`
   フィールドを足さず、スキーマ準拠のまま保つ。
...
3. 仮説として提示したリスクの前提は、エンベロープの `assumptions[]` に
   `{field, value, reason}` 形式で記録し、根拠が実データ（動的ナレッジ）
   ではなく一般的なパターンからの推定であることを `open_questions` にも
   反映する。リスク項目自体には `assumption` フィールドを足さず、スキーマ
   準拠のまま保つ。
