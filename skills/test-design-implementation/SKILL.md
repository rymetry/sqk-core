...
3. インラインで合成した TAE・そこから導出した COV・TC には `assumption:
   true` 相当の扱いを行い、「上流成果物なしのため、簡易 TAE をインライン
   合成した上で生成した」ことを `assumptions` に明記する。合成した TAE の
   `id` は `TAE-inline-001` のように区別可能な形にする。
