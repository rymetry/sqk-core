...
AI 活用は、**発想生成・チャーター草案・ログ要約・バグ報告ドラフト・優先度付け**の支援に加え、2023〜2026 年の LLM エージェント研究と汎用 computer-use 基盤の登場により、**探索セッションの実行そのもの**へ範囲が広がっています。機能理解に基づき GUI を自律探索する LLM エージェントは、従来の自動 GUI テスト手法を上回るカバレッジと欠陥検出を実測で示しました（GPTDroid、DroidAgent）。一方でベンチマーク研究は、意図生成・実行・欠陥検出の全サブタスクを高水準にこなせるモデルはまだ無いことも示しており（GUI Testing Arena）、NIST AI RMF や OECD の責任ある AI 指針が示す**信頼性・説明可能性・バイアス・人間の監督**を組み込まなければ、品質活動そのものの信頼を損ねる点は変わりません。本ナレッジベースの運用前提では、**探索の実行主体は AI エージェント（veridia 等の実行系）とし、人間は監督と最終判断（重大度・リリース可否）に責任を持ちます**。

このレポートの実務的結論は明確です。

探索的テストを組織能力にするには、次の三層で設計するのが最も再現性が高いです。

**戦略層ではチャーターとツアー、運営層では SBTM、実装層では AI エージェントによる探索実行と人間の監督を組み込む。** そのうえで、スモール探索的テストは局所的な入力・状態・境界・データの深掘りに、ラージ探索的テストは機能横断・業務フロー横断・リスク横断の探索に使い分けるのが合理的です。
...
timeline
    ...
    2023-2024 : LLM エージェントによる自律 GUI テストが実証される
    2025-2026 : 汎用 computer-use 基盤と探索的欠陥発見エージェントが登場し、探索実行主体が AI エージェントへ広がる
```

このタイムラインは、Afzal らの研究レビュー、Kaner/Bach の資料、SBTM の原典、Whittaker 書籍プレビュー、TMAP と現行ツール資料、および 2023〜2026 年の LLM エージェント研究（後述「実行主体としての AI エージェント」の出典）を統合して再構成したものです。
...
この型は、AI に事前要約・発想補助に加えてセッションの探索実行そのものを担当させてよい前提にしています（「実行主体としての AI エージェント」参照）。ただし、Xray の AI 実験が「草案作成には有効だが、妥当性確認は人間が必要」と報告しているとおり、重大度・優先度・リリース可否の最終判断は人間が担います。
...
AI の役割は、**観点生成と情報圧縮の補助者**から、**探索セッションを実行する主体**へと広がっています。2023〜2024 年の研究は、機能理解に基づいて GUI を自律探索する LLM エージェントが、従来の自動 GUI テスト手法を上回るカバレッジと欠陥検出を達成できることを実測で示し、2025〜2026 年には汎用 computer-use 基盤と探索的欠陥発見に特化したエージェント研究がこれに続きました（次節）。本ナレッジベースの運用前提もこれに合わせ、**探索の実行主体は AI エージェント（veridia 等の実行系）、人間は監督・デブリーフ・最終判断の担い手**とします。AI が強い領域として従来から挙げられてきた**アイデア生成、チャーター草案化、結果要約、文書化補助、視覚差分の機械支援**は、実行主体が変わっても補助層としてそのまま有効です。一方、探索が発見した事象の**価値判断・重大度判断・リリース可否の最終責任は依然として人間の責務**です。

### 実行主体としての AI エージェント

探索の実行そのものを AI エージェントへ委ねられるという判断は、次の出典に基づきます（出典検証は [docs/_research/exploratory-ai-execution](../_research/exploratory-ai-execution/README.md) レーン）。

- **GPTDroid**（Liu et al., ICSE 2024）は、GUI テストを LLM との Q&A として定式化し、機能認識メモリで探索を誘導することで、Google Play の 93 アプリにおいてベースライン比 +32% のアクティビティカバレッジと +31% の欠陥検出を報告し、新規欠陥 53 件（うち 35 件が確認・修正済み）を発見しました。
- **DroidAgent**（Yoon, Feldt, Yoo, 2023）は、長期・短期メモリを持つ自律エージェントが人手介入なしにタスク（意図）を自ら設定して探索し、従来最良手法を上回るカバレッジと、アプリ機能に照らして現実的なタスク生成を示しました。
- **GUITester**（Gao et al., 2026）は、タスク完遂を優先して異常を報告しない・システム欠陥を自身の誤操作と誤帰属するという GUI エージェント特有の2課題を整理し、探索的欠陥発見をベンチマーク（GUITestBench）で測定可能にしました。
- **汎用 computer-use 基盤**（例: Anthropic の computer use tool）は、スクリーンショット・マウス・キーボード操作によるデスクトップ環境の自律操作を LLM に与えるもので、GUI 探索の実行系を特定のテストツールに依存せず構成できることを示しています（beta 段階であり、ベンダー各社もセキュリティ上の考慮と人間の監督を明記しています）。

同時に、**GUI Testing Arena**（Zhao et al., 2024）は、自律 GUI テストを意図生成・タスク実行・欠陥検出の3サブタスクに分けて測定し、最先端モデルでも全サブタスクを高水準にはこなせないと報告しています。GUITester の実測値（F1 50% 未満）も同様に、探索実行の自律化が「可能」であることと「無監督で信頼できる」ことの間に距離があることを示します。したがって役割境界は次のとおりです。

| 役割 | 担い手 |
| --- | --- |
| チャーター選定・セッション設計・デブリーフ後処理のブループリント | 本ナレッジベース（sqk-core のスキル） |
| 探索セッションの実行・操作・証跡収集 | AI エージェント（veridia 等の実行系） |
| セッション証跡・異常報告のレビュー、価値判断・重大度判断・リリース可否 | 人間 |
...
flowchart LR
    ...
    F --> H
    G --> H
    H --> I[セッション実行<br>AIエージェント または 人間]
    I --> J[証跡とレポート]
    J --> K[人間によるデブリーフ・最終判断]
```

この構成は、mabl・TestRail・Xray・Applitools の現行機能と、NIST の AI TEVV / AI RMF の考え方、および前節の LLM エージェント研究を合わせた実装像です。セッション実行を AI エージェントが担う場合も、実行前の承認と実行後のデブリーフは人間が行います。
...
- **人間の監督**
発見の価値判断、重大度判断、リリース判断は人間が責任を持つ。探索実行を AI エージェントに委ねる場合も、エージェントにはシステム欠陥を自身の誤操作と誤帰属する・タスク完遂を優先して異常を報告しないという既知の失敗様式があるため（GUITester の課題整理）、セッション証跡と異常報告のレビューは人間が行う。
